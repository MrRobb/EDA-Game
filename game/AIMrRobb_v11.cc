...
struct PLAYER_NAME : public Player {
	...
	int habitantes_path(int path_id)
	{
		vector<int> hab(nb_players(), 0);
		for (int i = 0; i < nb_units(); i++)
		{
			if (cell(unit(i).pos).path_id == path_id)
			{
				if (unit(i).player >= 0 and unit(i).player < nb_players())
				{
					hab[unit(i).player]++;
				}
			}
		...
		int m = *max_element(hab.begin(), hab.end());
		if (me() >= 0 and me() < nb_players()) m -= m - hab[me()];
		return m > 0 ? m + 1 : 1;
	}
	
	int habitantes_city(int city_id)
	{
		vector<int> hab(nb_players(), 0);
		for (int i = 0; i < nb_units(); i++)
		{
			if (cell(unit(i).pos).city_id == city_id)
			{
				if (unit(i).player >= 0 and unit(i).player < nb_players())
				{
					hab[unit(i).player]++;
	...
	static bool compPath(pair<Pos, int> p1, pair<Pos, int> p2)
	{
		return p1.second < p2.second;
	}
	...
	void which_cities(int ork, queue<Pos> &q)
	{
		...
		for (int i = 0; i < nb_cities(); i++)
		{
			...
			cities[i] = make_pair(min, (manhattan_distance(min, unit(ork).pos) + isla(city(i))) * habitantes_city(i));
		}
	...
	void which_paths(int ork, queue<Pos> &q)
	{
		...
		for (int i = 0; i < nb_paths(); i++)
		{
			...
			paths.push_back(make_pair(min, manhattan_distance(min, unit(ork).pos) * habitantes_path(i)));
		}
		
		sort(paths.begin(), paths.end(), compPath);
	...
	Dir runaway(Pos myself, Pos enemy, int ork) {
		if (cell(myself).city_id != -1) {
			...
			Pos max = c[0];
			for (auto p : c) {
				if (manhattan_distance(p, enemy) > manhattan_distance(max, enemy)) {
					max = p;
				}
			}
